...
class HocaPanelShowAlertWidget extends StatelessWidget {
  const HocaPanelShowAlertWidget({required HocaPanelViewModal state,required this. hoca,super.key}) : _state = state;
  final HocaPanelViewModal _state;
  final List<dynamic> hoca;
  @override
                  ...
                  return NormalButton(
                    onPressed: _state.isReadyComfirm ? () async {
                         if(_state.newPasswordController.text == _state.newRepeatPasswordController.text)
                             {
                     await SqlUpdateService.updateKullaniciSifre("${hoca[0]}",_state.newPasswordController.text,SqlTableName.hoca);
                           Navigator.of(context).pop();
                            _state.alertResetButton();
                            _state.resetButtonState();
                            _state.comfirmButtonState();
